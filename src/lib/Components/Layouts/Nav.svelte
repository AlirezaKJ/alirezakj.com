...
<nav class="flex justify-end items-center w-full h-36 px-6 z-1">
  <div class={`fixed top-0 transition-all duration-500 z-1 ${ontop || onbottom ? 'left-30' : 'left-6'}`}> <!-- 6 (24) + 12 (48) + 12 (48) = 120 -->
    <a href="/" on:click={scrollToTop} class="font-serif flex justify-center items-center h-36">
      ...
      class={`h-12 w-12 ${ontop || onbottom ? 'opacity-0' : 'opacity-100'}  overflow-hidden flex justify-center ${scrollingDirection == 1 ? 'rotate-90' : ''} ${scrollingDirection == -1 ? '-rotate-90' : ''} items-center origin-[-24px_24px]`} id="arrow">
